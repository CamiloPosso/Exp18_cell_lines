...
server <- function(input, output, session) {
  ...
  output$plot1 <- renderPlot({
    plot_function(appdata(), chosen_DB(), treatments(), "sim", p_cutoff(), n_pathways(), exclude_rest())
  })
  output$plot2 <- renderPlot({
    plot_function(appdata(), chosen_DB(), treatments(), "diff", p_cutoff(), n_pathways(), exclude_rest())
  })
  
  output$summaryplot <- renderPlot({
    p = venn_plot_function(appdata(), chosen_DB(), p_cutoff())
    print(p)
  })
  ...
  output$save_plot <- downloadHandler(
    ...
    content = function(file) {
      p <- reactive({
        plot_function(appdata(), chosen_DB(), treatments(), save_mode(), p_cutoff(), n_pathways(), exclude_rest())
      })
