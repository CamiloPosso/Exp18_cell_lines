...
for (t2g_name in names(t2gs)){
  print(t2g_name)
  t2g <- t2gs[[t2g_name]]
  gsea_input <- m_global_components[["feature_decomposition"]]$PC1
  names(gsea_input) <- m_global_components[["feature_decomposition"]] %>% rownames()
  ...
  gsea_input <- m_global_components[["feature_decomposition"]]$PC2
  names(gsea_input) <- m_global_components[["feature_decomposition"]] %>% rownames()
...
samplewise_gsea <- function(m, t2g, table_name = "", normalize = TRUE){
  ...
  write.table(sample_gsea, paste0("sample_gsea_", table_name, ".txt"), sep = "\t", quote = F)
  return(sample_gsea)
}
...
samplewise_ksea <- function(m){
  ...
  write.table(sample_ksea, "sample_ksea_psp_networkin5.txt", sep = "\t", quote = F)
}
...
```{r}
functional_pca_helper <- function(sample_gsea, group_gsea, gsea_meta, arrow_pathways = NULL, top_n = 8){
  ...
  p <- plot_pca(m_gsea, "Treatment", z_score = F, standardize = T, shape = "State",
                save_dfs = "PCA_of_functional_enrichment") + ggtitle("PCA of GSEA enrichment") 
  ...
  arrow_df <- PCA_of_functional_enrichment$feature_decomposition %>%
    mutate(arrow_len = sqrt(PC1**2 + PC2**2),
           Pathway = rownames(.),
           label_name = Pathway)
  if (is.null(arrow_pathways)){
    arrow_df <- arrow_df %>%
      arrange(-arrow_len) %>% head(top_n) %>%
      select(x_c = PC1, y_c = PC2, label_name, Pathway)
  } else {
    arrow_df <- arrow_df %>%
  ...
  confirm_pca_og <- group_gsea %>%
    filter(Pathway %in% arrow_df$Pathway, 
           p.adjust < 0.05) %>% select(Pathway, enrichment, p.adjust, Treatment) %>%
    left_join(arrow_df, by = "Pathway")
  
  q <- arrow_plotter(arrow_df, top_n)
...
DB_name = "reactome"
sample_gsea <- read.table(paste0("sample_gsea_", DB_name, ".txt"), sep = "\t") %>%
...
gsea_meta <- pData(m_global) %>% select(Sample, Treatment = Treatment, State)
rownames(gsea_meta) <- gsea_meta$Sample

output <- functional_pca_helper(sample_gsea, group_gsea, gsea_meta)



## KSEA

sample_ksea <- read.table("sample_ksea_psp_networkin5.txt", sep = "\t") %>% mutate(Pathway = pathway)
group_ksea <- read.table("../comparisons/4.5_exp12_combined_states_combined_functional_diffsim_treatments/KSEA_combined.txt", sep = "\t")

ksea_meta <- pData(m_phospho) %>% select(Sample, Treatment = Treatment, State)
rownames(ksea_meta) <- ksea_meta$Sample

output <- functional_pca_helper(sample_ksea, group_ksea, ksea_meta)
