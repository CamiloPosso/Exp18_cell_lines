...
for (k in 1:length(omics)) {
  ...
  if (grepl("global", omics[k], ignore.case = TRUE)) {
    ...
    # panSEA.results <- panSEA::panSEA(data.list, types, GSEA = FALSE,
    #                                  DMEA.type = "cell_corr",
    ...
    prot.panSEA.results <- panSEA::panSEA(data.list, types, GSEA = FALSE, 
                                          DMEA.type = "cell_corr",
                                          GSEA.rank.var = rep("Chr8", length(types)),
                                          group.names = "Chr8", group.samples = 2,
                                          feature.names = feature.names,
    ...
    all.files <- list('DMEA_cell_corr_with_proteomics' = prot.DMEA.files)
    # all.files <- list('GSEA' = GSEA.files,
    #                   'DMEA_cell_corr' = DMEA.files,
    #                   'DMEA_cell_corr_with_proteomics' = prot.DMEA.files)
    } else {
      # only 53 gene names for "global" (without mouse)! so skipping GSEA
      # using proteomics as query database
      prot.panSEA.results <- list()
      prot.panSEA.results[["mDMEA.results"]] <- 
        panSEA::mDMEA_cell_corr(weights = data.list, types = types, 
                      expression = expr)
